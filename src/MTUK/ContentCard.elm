module MTUK.ContentCard exposing (model, update, view, Model, Msg (Init))
...
update action model =
    case action of
        ...
        ApiResponse (API.UpdateFailure x) ->
            (model, Cmd.none) -- TODO: report login errors, low priority
        Init ->
...
view model = case model.body of
    Nothing ->
        Card.view options
            [ Card.title [] [ Card.head [] [ text model.title ] ]
